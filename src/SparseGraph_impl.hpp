...
SparseGraph<T>::generateAdjacencyMatrix(const T g) {
  std::shared_ptr<const T> adjacencyMatrix = std::make_shared<const T>(g);
...
SparseGraph<DenseMat>::generateDegreeMatrix(
  ...
  std::size_t n = adjacencyMatrix->rows();
  auto degreeMatrix = std::make_shared<DenseMat>(DenseMat::Zero(n, n));
  for (std::size_t i = 0; i < n; ++i) {
...
SparseGraph<SparseMat>::generateDegreeMatrix(
  ...
  std::size_t n = adjacencyMatrix->rows();
  auto degreeMatrix = std::make_shared<SparseMat>(SparseMat(n, n));
  for (std::size_t i = 0; i < n; ++i) {
...
template <>
const std::shared_ptr<const DenseMat>
SparseGraph<DenseMat>::generateAdjacencyMatrix(
...
template <>
const std::shared_ptr<const SparseMat>
SparseGraph<SparseMat>::generateAdjacencyMatrix(std::ifstream &file) {
  std::string ind;
  std::string line;
